...
import { OPEN_BOOKING_EVENT } from "@/lib/booking-trigger";
...
export default function BookingStickyBar({ maxGuests = 12, cta = "Book" }: BookingStickyBarProps) {
  ...
  // Page "Book an appointment" CTAs dispatch OPEN_BOOKING_EVENT. If a date is
  // chosen, open the details overlay; otherwise highlight the date field.
  useEffect(() => {
    const onOpen = () => {
      if (date) {
        setExpanded(true);
        ...
        try { (dateRef.current as any)?.showPicker?.(); } catch { /* unsupported */ }
        setPulse(true);
  ...
  return (
    <>
      ...
      <div
        id="booking-sticky-bar"
        className={`fixed bottom-0 inset-x-0 z-40 bg-cream/95 backdrop-blur-md border-t shadow-lg transition-all duration-300 ${
          pulse ? "border-sage ring-2 ring-sage/40" : "border-stone/10"
        }`}
      >
        <div className="container mx-auto px-4 py-3 max-w-5xl flex items-center gap-2 sm:gap-3">
          <label className="flex-1 min-w-0 bg-stone/5 border border-stone/10 rounded-lg px-3 py-2 focus-within:border-stone/40 transition-colors">
            ...
            <input ref={dateRef} type="date" min={today} value={date} onChange={(e) => setDate(e.target.value)} className="w-full bg-transparent text-sm text-stone outline-none" />
          </label>
